...
Frontend em **React + TypeScript (Vite)** que consome a API do backend Spring
Boot deste repositório.

> Parte do monorepo **Car Workshop** — o backend fica em
> [`../backend`](../backend). Veja o [README da raiz](../README.md) para a visão
> geral.
...
O backend **não** expõe configuração de CORS. Em desenvolvimento isso é resolvido
pelo **proxy do Vite** (ver `vite.config.ts`): o navegador faz requisições para a
mesma origem (`:5173`) e o Vite as repassa ao backend (`:8080`).

Em **produção**, com frontend e backend em origens diferentes, habilite CORS no
backend — por exemplo:
